...
const SubjectCylinder = () => {
    ...
    useEffect(() => {
        const fetchStudentSubjects = async () => {
            ...
            try {
                ...
                const fetchedCourses = response.data.map(subject => ({
                    id: subject.id,
    ...
    const currentSubject = courses.length > 0 ? courses[activeCourseIndex] : null;
    const currentSubjectId = currentSubject ? currentSubject.id : null;
    const currentSubjectName = currentSubject ? currentSubject.title : null;
    ...
    return (
        ...
        >
            ...
            {loading ? (
                ...
                <div className="relative flex items-center py-35 justify-center h-50 perspective-1000">
                    <button
                    ...
                    >
                        {courses.map((course, index) => {
                            ...
                            const isPrev =
                                index === (activeCourseIndex - 1 + courses.length) % courses.length;
                            const isNext =
                                index === (activeCourseIndex + 1) % courses.length;
                            ...
                            return (
                                <div
                                    key={course.id}
                                    className={`absolute p-5 rounded-lg border shadow-lg flex flex-col justify-between overflow-hidden transition-all duration-500 ease-in-out ${bgStyle} ${
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <div
                                            className={`rounded-full h-2.5 mb-1 overflow-hidden ${
            ...
            {currentSubjectId && currentSubjectName && (
                <LessonCylinder subjectId={currentSubjectId} subjectName={currentSubjectName} />
            )}
...
export default SubjectCylinder;
